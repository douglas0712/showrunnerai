...
export function reconcileNarrationJobs({ db = null } = {}) {
  const banco = db;
  // ── por que o filtro é por WORKFLOW, e não por `kind` ───────────────────
  //
  // Porque `kind='audio'` deixou de significar "narração". Desde o PASSO 14-D1B
  // o efeito sonoro também é áudio — e roda no ComfyUI, com fila e histórico
  // próprios. Varrer todo áudio aberto aqui marcaria como órfão um efeito que
  // está VIVO na GPU neste instante, e o resultado dele chegaria a um job que o
  // Showrunner já tinha dado como perdido.
  //
  // Esta reconciliação é só do sintetizador de voz, e o que identifica a voz é
  // o `workflowId` que ela mesma grava.
  const abertos = banco.prepare(`
    SELECT jobId FROM generation_jobs
     WHERE kind = 'audio' AND workflowId = ?
       AND state NOT IN ('done', 'failed', 'cancelled', 'orphaned')
     ORDER BY createdAt ASC
  `).all(NARRATION_WORKFLOW_ID);
