 ...
 * da tolerância. Uma validação feita no meio da escrita transformaria "este
 * plano está errado" em "metade deste plano está gravada".
 *
 * ── Por que a transação abre ANTES da conferência ───────────────────────────
 *
 * Porque uma das conferências não é sobre o que veio: é sobre o que o banco
 * tem. `recusarSeJaTemMidia` pergunta se alguma cena já ganhou mídia, e a
 * resposta pode mudar entre a pergunta e o DELETE.
 *
 * Fora da transação, a janela é real: a guarda encontra zero takes, outra
 * conexão grava um, e o DELETE que vem em seguida o leva junto pelo CASCADE —
 * a destruição exata que a guarda existe para impedir, e silenciosa, porque
 * ninguém viu o take existir.
 *
 * `BEGIN IMMEDIATE` toma o lock de escrita na hora, e não no primeiro INSERT.
 * Abrindo a transação primeiro, a guarda lê um banco que mais ninguém pode
 * mudar até o COMMIT: entre "não há mídia" e "as cenas foram apagadas" não cabe
 * mais nada. Qualquer outra conexão que tente gravar nesse intervalo esbarra em
 * SQLITE_BUSY e falha — que é o resultado certo, porque a alternativa é perder
 * o trabalho dela sem avisar.
 *
 * O custo é o lock ficar de pé durante a validação, que é aritmética sobre no
 * máximo 40 cenas em memória. É barato, e é o preço de a guarda valer.
 *
 * A ordem dentro da transação preserva a prioridade decidida no 13-A.1: a mídia
 * é conferida primeiro, então a mensagem de proteção continua vencendo a de
 * duração inválida. Recusar por duração ensinaria a quem chama que basta
 * acertar os segundos para a mídia sumir.
 */
export function replaceProductionScenes(projectId, cenas = [], db = database()) {
  const { plano, roteiro } = exigirRoteiro(projectId, db);

  // O lock de escrita vem PRIMEIRO. Ver o cabeçalho: a guarda de mídia lê o
  // banco, e o que ela lê precisa continuar valendo até o DELETE.
  db.exec('BEGIN IMMEDIATE');
  try {
    // Antes de tudo, inclusive antes de conferir o que veio: se esta produção
    // já tem mídia, substituir o conjunto é destruição, e destruição não passa
    // por aqui. Ver `recusarSeJaTemMidia`.
    recusarSeJaTemMidia(roteiro, db);
    ...
    // Os ordinais são exatamente 1..n. "Sem repetido" e "sem buraco" são a
    // mesma conferência: um conjunto de n posições distintas entre 1 e n é 1..n.
...
/**
 * Recusa a substituição em bloco quando alguma cena já tem mídia.
 *
 * ── Por que isto existe ─────────────────────────────────────────────────────
 *
 * `replaceProductionScenes` apaga as cenas e as reinsere com `id` novo. Isso
 * era inofensivo enquanto uma cena era só texto. Desde o PASSO 13-A não é: a
 * mídia da cena pende do `id` dela, e o CASCADE levaria takes e seleções junto
 * — em silêncio, e sem nada para desfazer. Um pedido de "reescreva o roteiro"
 * viraria a destruição de uma tarde de geração de imagem.
 *
 * ── Por que RECUSAR, e não preservar por ordinal ────────────────────────────
 *
 * Porque o ordinal não é identidade: depois de uma reescrita, a cena 4 pode ser
 * outra cena. Reatar a imagem antiga ao número 4 colaria a mídia da cena errada
 * no lugar certo — um defeito que passa despercebido justamente por parecer
 * certo. Preservar bem exige saber qual cena virou qual, e isso é um fluxo de
 * replanejamento explícito, não um efeito colateral desta função.
 *
 * ── Por que não há force ────────────────────────────────────────────────────
 *
 * Um parâmetro de contorno seria um parâmetro que o modelo aprende a mandar
 * quando a primeira tentativa falha — e o agente não tem como pesar o que o
 * usuário perde. Enquanto o fluxo explícito não existe, a resposta é não, para
 * todo mundo. Editar uma cena de cada vez continua liberado, e é o caminho que
 * o erro aponta.
 *
 * A mensagem não cita `id` de cena, de mídia nem de Asset: quem lê está numa
 * conversa, e o que ele precisa saber é o que fazer em vez disso.
 */
function recusarSeJaTemMidia(roteiro, db) {
  // Desde o PASSO 14-B a mídia de uma cena não é só o que se vê: um take de
  // narração é uma tentativa de voz que alguém pediu, e o CASCADE o levaria
  // exatamente como leva uma imagem. Contar só a tabela visual deixaria a
  // guarda passar em silêncio numa produção que só tem voz — a cena sem imagem
  // nenhuma e com três leituras gravadas é justamente o caso em que a perda
  // dói e ninguém vê acontecer.
  //
  // As duas famílias entram na MESMA soma, por ordinal, porque a pessoa que lê
  // o erro não precisa saber de qual tabela veio o que ela está prestes a
  // perder: ela precisa saber em quais cenas há trabalho feito.
  const comMidia = db.prepare(`
    SELECT ordinal, SUM(takes) AS takes FROM (
      SELECT c.ordinal AS ordinal, COUNT(*) AS takes
        FROM production_scene_media m
        JOIN production_scenes c ON c.id = m.sceneId
       WHERE c.scriptId = ?
       GROUP BY c.ordinal
      UNION ALL
      SELECT c.ordinal AS ordinal, COUNT(*) AS takes
        FROM production_scene_audio_takes a
        JOIN production_scenes c ON c.id = a.sceneId
       WHERE c.scriptId = ?
       GROUP BY c.ordinal
      UNION ALL
      -- PASSO 14-D1A: uma CUE de efeito conta mesmo SEM take.
      --
      -- Ela é diferente das outras duas famílias de propósito. Um take de
      -- imagem ou de voz é uma tentativa de produzir algo; uma cue é a DECISÃO
      -- de que a cena tem aquele som — texto que alguém escreveu, pensando no
      -- desenho sonoro. Esperar o take existir para proteger deixaria uma
      -- reescrita apagar, em silêncio, uma tarde inteira de sound design que
      -- ainda não virou arquivo.
      SELECT c.ordinal AS ordinal, COUNT(*) AS takes
  ...
  `).all(roteiro.id, roteiro.id, roteiro.id);

  if (comMidia.length === 0) return;

  const numeros = comMidia.map((linhaDaCena) => Number(linhaDaCena.ordinal));
  const total = comMidia.reduce((soma, linhaDaCena) => soma + Number(linhaDaCena.takes), 0);

  throw new DomainError(
    'Não é possível substituir todas as cenas porque esta produção já tem mídia '
    + `associada (cena${numeros.length > 1 ? 's' : ''} ${numeros.join(', ')}). `
    + 'Edite as cenas uma a uma, ou use um fluxo explícito de replanejamento.',
    { cenasComMidia: numeros, takes: total },
  );
}
