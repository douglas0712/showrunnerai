...
export function createActiveTurnRegistry() {
  ...
  return {
    ...
    end(sessionId) { ativos.delete(String(sessionId)); },
...
export async function handleBridgeInvocation(requisicao = {}, {
  ...
  try {
    ...
    if (erro instanceof UnknownToolAliasError) {
      return recusa('unknown_tool', 'Ferramenta desconhecida.');
    }
    throw erro;
  }
  ...
  try {
    ...
    if (erro instanceof UnknownRuntimeSessionError || erro instanceof DomainError) {
      return recusa('unknown_session', 'Sessão desconhecida.');
    }
    throw erro;
  }
  ...
  if (!thread) return recusa('unknown_thread', 'Conversa desconhecida.');
  ...
  try {
    ...
    return { ok: true, result: sanitizarResultado(resultado) };
  } catch (erro) {
