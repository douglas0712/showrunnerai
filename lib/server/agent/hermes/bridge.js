...
//
// `projectId`, `threadId` e `userMessageId` NÃO chegam, e o desenho depende
// disso. Eles são deduzidos aqui, do que o servidor já sabia: a associação
// gravada quando a sessão foi criada, e o turno que o adaptador anunciou. Se
// chegassem pelo canal, seriam entrada controlada pelo modelo, e a checagem de
// propriedade do PASSO 6 passaria a validar um número que o próprio chamador
// escolheu — ou seja, deixaria de validar.
//
// A requisição é DESESTRUTURADA em três campos e mais nenhum: o que vier além
// deles não é recusado, é ignorado, e ignorar é mais forte do que recusar —
// não há caminho pelo qual um quarto campo passe a ser lido por engano.
//
...
//
//   1. schema do Hermes    o modelo só enxerga as tools do toolset showrunner
//                          (medido: 1 tool no catálogo com o toolset restrito)
//   2. allowlist do plugin o plugin registra uma lista fechada de nomes
...
/**
 * Tools que exigem um projeto para rodar.
 *
 * Todas, hoje — e a lista continua explícita em vez de virar "sempre": uma
 * ferramenta que NÃO precise de projeto é concebível (perguntar as capacidades
 * da instalação, por exemplo), e a lista é onde essa decisão apareceria.
 *
 * As duas de documento entram aqui porque o dono de um documento é o Project:
 * sem projeto não há o que listar nem o que ler, e a recusa precisa acontecer
 * antes de a ferramenta rodar — não dentro dela, com o contexto pela metade.
 */
const EXIGEM_PROJETO = Object.freeze([
  'og.generate_image', 'og.generate_video', 'og.get_job',
  'project.list_documents', 'project.read_document',
]);
 ...
 *
 * ── Por que ele guarda mais do que o sinal ──────────────────────────────────
 *
 * Este é o único lugar do caminho do plugin que sabe QUAL turno está em curso.
 * A sessão diz de que conversa se trata; ela não diz de que TURNO — e a mesma
 * conversa tem muitos. Quem sabe isso é quem anunciou o turno.
 *
 * Por isso o registro guarda o contexto mínimo do turno, e não só o sinal. Hoje
 * são dois campos; o segundo é a mensagem do usuário que iniciou o turno, que é
 * a âncora durável dele — ela já está gravada no banco antes de qualquer
 * ferramenta poder rodar.
 *
 * Nada disso chega pelo socket. Se chegasse, seria entrada controlada pelo
 * modelo, e um agente capaz de dizer a que turno pertence o trabalho que ele
 * mesmo pediu é um agente sem fronteira — a mesma razão pela qual `threadId` e
 * `projectId` também são deduzidos aqui.
 */
export function createActiveTurnRegistry() {
  ...
  return {
    /**
     * Anuncia um turno. `turno` é o contexto mínimo dele, construído no
     * servidor — nunca recebido do runtime nem do modelo.
     */
    begin(sessionId, { signal = null, userMessageId = null } = {}) {
      ativos.set(String(sessionId), {
        signal: signal ?? null,
     ...
     * Desanuncia o turno.
     *
     * Uma saída só, chamada do `finally` do adaptador: conclusão, falha,
     * cancelamento e teto passam todos por aqui. O contexto não pode sobreviver
     * ao turno — um identificador de turno preso num registro global é
     * exatamente o tipo de coisa que a chamada seguinte herdaria sem ninguém
     * notar.
     */
    end(sessionId) {
    ...
    /** O contexto do turno em curso desta sessão, ou `null`. */
    turnFor(sessionId) { return ativos.get(String(sessionId)) ?? null; },

    signalFor(sessionId) { return ativos.get(String(sessionId))?.signal ?? null; },
...
export async function handleBridgeInvocation(requisicao = {}, {
  ...
  //    pelo canal.
  //
  //    `threadId` e `projectId` saem do vínculo da sessão; `userMessageId` e o
  //    sinal saem do turno que o adaptador anunciou. Fora de um turno anunciado
  //    não há âncora, e `null` é a resposta honesta: nenhuma é inventada.
  const turno = turns?.turnFor(vinculo.sessionId) ?? null;
  const contexto = {
    threadId: thread.id,
    projectId: thread.projectId,
    userMessageId: turno?.userMessageId ?? null,
    signal: turno?.signal ?? null,
  };
