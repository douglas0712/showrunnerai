...
export function handleCreateThread(corpo, deps = {}) {
  ...
  return executar(() => {
    ...
    // Uma conversa recém-criada não tem produção nenhuma — mas o campo vem
    // mesmo assim, para que quem consome não precise distinguir "vazio" de
    // "ausente" conforme a rota que respondeu.
    return { status: 201, body: { ...agentIdentity, thread, messages: [], production: [] } };
  });
}
...
export function handleGetThread(threadId, deps = {}) {
  ...
  // `production` é o que esta conversa tem em andamento AGORA — tipo e estado,
  // nada mais. É por aqui que a tela descobre que há uma imagem sendo gerada
  // depois de o turno já ter acabado, sem que exista uma segunda API para isso:
  // a mesma leitura traz as mensagens, a mídia e o que ainda está em produção.
  //
  // Perguntar não faz o trabalho andar. Quem faz é o acompanhamento do
  // servidor; isto aqui só olha.
  return executar(() => {
    const { thread, messages, production } = getThread(threadId, deps);
    return { status: 200, body: { ...agentIdentity, thread, messages, production } };
  });
}
...
export async function handleSendMessage(corpo, deps = {}) {
  ...
  const documentIds = listaDeIds(corpo.documentIds);
  if (documentIds === null) return erro(400, 'documentIds inválido.');

  return executarAsync(async () => {
    const turno = await sendMessage({ threadId, content, documentIds }, deps);
    return {
      status: 200,
      body: {
        ...agentIdentity,
...
export function handleStreamMessage(corpo, deps = {}) {
  ...
  return {
    ...
    stream: (async function* () {
      try {
        for await (const evento of streamMessage({ threadId, content, documentIds, ...(deps.signal ? { signal: deps.signal } : {}) }, deps)) {
...
/**
 * Os documentos que o navegador diz ter anexado a este turno.
 *
 * Aqui só a FORMA é conferida — uma lista curta de strings. Se aquele
 * documento existe, se é deste projeto e se pode ser ligado a esta mensagem é
 * decisão do domínio, e é lá que ela mora (`attachMessageDocuments`): a
 * validação de forma numa borda HTTP nunca deve ser confundida com a checagem
 * de propriedade, porque só a segunda protege alguma coisa.
 *
 * Devolve `null` quando a forma está errada — distinto de `[]`, que é "nenhum
 * anexo" e é perfeitamente válido.
 */
function listaDeIds(valor) {
  if (valor === undefined || valor === null) return [];
  if (!Array.isArray(valor) || valor.length > MAX_ANEXOS_POR_TURNO) return null;
  const saida = [];
...
/**
 * Teto de anexos num turno.
 *
 * Não é limitação de produto, é limite de superfície: sem teto, um cliente
 * mandaria dez mil ids e cada um viraria uma consulta ao banco antes de ser
 * descartado. Doze é folgado para o que um humano anexa de uma vez.
 */
const MAX_ANEXOS_POR_TURNO = 12;
