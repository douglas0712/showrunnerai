...
//
// E, dentro do WebSocket, quatro métodos:
//
//     session.create      abre a conversa do lado do runtime
//     session.resume      reabre, pelo id durável, uma conversa que o runtime
//                         reciclou — devolve um id vivo novo para a MESMA
//                         conversa, com o histórico dela
...
/**
 * Por que restabelecer a sessão é uma coisa que precisa ser detectada.
 *
 * O runtime recicla sozinho: uma sessão cujo WebSocket criador se desconectou e
 * que não está executando nada é recolhida depois de uma janela de carência
 * (`HERMES_TUI_WS_ORPHAN_REAP_GRACE_S`, padrão 20 s — `_schedule_ws_orphan_reap`).
 * Como esta integração abre um socket por RPC e o fecha em seguida, TODA sessão
 * nossa é órfã desde que nasce, e uma pausa de meio minuto entre duas falas do
 * usuário basta para o identificador guardado deixar de existir.
 *
 * Distinguir esse caso é a diferença entre "o runtime está fora do ar" e "a
 * conversa dele foi reciclada". A primeira é um problema da instalação, e a
 * resposta certa é dizer ao usuário que tente mais tarde; a SEGUNDA é rotina, e
 * a resposta certa é restabelecer e seguir — sem que o usuário fique sabendo
 * que existe uma sessão.
 */

/**
 * O código que o runtime usa para recusar por pré-condição.
 *
 * ── Ele NÃO identifica a falha sozinho ──────────────────────────────────────
 *
 * Auditado no fonte da v0.20.3: `4001` é genérico e aparece em mais de vinte
 * lugares, para condições que não têm nada a ver com sessão — "malformed server
 * config", "pcm frame too large", "slug is required", "invalid base64 pcm". Ele
 * é o "400" deste protocolo, não um código de sessão.
 *
 * Dentro de `prompt.submit`, hoje, ele tem uma origem só (`_sess_nowait`, que
 * responde `4001 "session not found"`) — as outras recusas daquele método usam
 * 4090, 4009, 4004 e 4030. Mas isso é um detalhe INTERNO de um método de
 * terceiro, e apostar nele significaria que um `4001` novo acrescentado ali no
 * futuro viraria uma retomada de sessão silenciosa e errada.
 */
const CODIGO_RECUSA_POR_PRECONDICAO = 4001;

/**
 * A frase exata com que o runtime nomeia esta condição (`server.py`,
 * `_sess_nowait`). Comparada em minúsculas e por conteúdo, para que pontuação
 * ou um sufixo com o id não estraguem o reconhecimento.
 */
const MOTIVO_SESSAO_DESCONHECIDA = 'session not found';

/**
 * A falha é "esta sessão não existe mais"?
 *
 * ── Três condições, e todas precisam valer ──────────────────────────────────
 *
 *   rotulo   a recusa veio de `prompt.submit`. É um valor NOSSO, escrito por
 *            esta camada — a parte mais confiável da checagem.
 *   codigo   a recusa é por pré-condição, e não um erro de transporte.
 *   motivo   e a pré-condição é a sessão. É o que separa esta falha das outras
 *            que compartilham o mesmo código.
 *
 * ── Por que o texto entra na conta ──────────────────────────────────────────
 *
 * Porque o código é ambíguo, e a alternativa seria confiar em qual `_err` um
 * método de terceiro chama internamente. Comparar texto de terceiro é frágil e
 * normalmente se evita; aqui ele é o único campo estruturado que resta, e entra
 * em conjunção com dois sinais robustos.
 *
 * O modo de falhar dessa fragilidade foi escolhido: se o runtime reescrever a
 * frase, esta função devolve `false`, o turno falha como falhava antes, e
 ...
 */
export function isSessaoDesconhecida(erro) {
  if (!(erro instanceof HermesTransportError)) return false;
  const { rotulo, codigo, motivo } = erro.detail ?? {};
  return rotulo === 'prompt.submit'
    && codigo === CODIGO_RECUSA_POR_PRECONDICAO
    && String(motivo ?? '').toLowerCase().includes(MOTIVO_SESSAO_DESCONHECIDA);
}
...
export function createHermesClient({
  ...
  async function* falar({ aoAbrir, terminou, signal = null, rotulo }) {
    ...
    try {
      ...
      while (!encerrado) {
        ...
        if (erro) {
          // `motivo` é o texto do runtime, guardado no DETAIL — nunca na
          // mensagem. Ele existe porque o código sozinho não identifica a
          // falha (ver `isSessaoDesconhecida`), e porque é o que um operador
          // precisa ler no log. A frase pública continua sendo a nossa.
          throw new HermesTransportError('O runtime recusou a requisição.', {
            rotulo,
            codigo: erro.code ?? null,
            motivo: typeof erro.message === 'string' ? erro.message : null,
          });
        }
  ...
  return {
    ...
    /**
     * Reabre uma conversa que o runtime reciclou, pelo id DURÁVEL dela.
     *
     * ── Por que isto não é criar outra sessão ───────────────────────────────
     *
     * `session.create` começa do zero. `session.resume` reencontra a conversa
     * no armazenamento durável do runtime e devolve um identificador VIVO novo
     * para a MESMA conversa — com o histórico dela.
     *
     * A diferença é tudo, porque nesta integração o histórico do turno anterior
     * mora do lado de lá: o adaptador entrega ao runtime apenas a última fala
     * do usuário. Recriar a sessão faria "resuma esse documento" chegar a um
     * modelo que nunca viu documento nenhum.
     *
     * ── O que muda e o que não muda ─────────────────────────────────────────
     *
     * Muda o identificador do GATEWAY — o efêmero, por onde a fala é enviada.
     * NÃO muda o durável, que é o nome pelo qual o plugin reconhece a sessão
     * quando o modelo chama uma ferramenta. É por isso que restabelecer não
     * quebra a ponte de ferramentas: o lado que o plugin conhece é estável.
     *
     * `omit_messages` pede ao runtime que não devolva o transcrito inteiro pelo
     * socket. O histórico é materializado do mesmo jeito para o modelo — é o
     * que o runtime documenta —, e trazê-lo de volta seria transportar,
     * sem uso, tudo o que já foi dito na conversa.
     */
    async resumeSession({ storedSessionId, signal = null } = {}) {
      const alvo = String(storedSessionId || '').trim();
      if (!alvo) {
        throw new HermesTransportError('Não há sessão durável para reabrir.', {});
      }

      let sessionId = null;
      let sessionKey = null;
      for await (const quadro of falar({
        rotulo: 'session.resume',
        signal,
        aoAbrir: (enviar) => enviar(
          'session.resume',
          { session_id: alvo, omit_messages: true },
          1,
        ),
        terminou: (quadro) => quadro?.id === '1',
      })) {
        if (quadro?.id !== '1') continue;
        sessionId = quadro?.result?.session_id ?? null;
        sessionKey = quadro?.result?.session_key ?? null;
      }
      if (!sessionId) {
        throw new HermesTransportError('O runtime não reabriu a conversa.', {});
      }
      return {
        sessionId: String(sessionId),
        // O durável continua sendo o mesmo; o runtime o confirma em
        // `session_key`. Cair no que pedimos é o padrão correto — é o valor que
        // o banco já guarda e o que o plugin já usa.
        bridgeSessionId: sessionKey ? String(sessionKey) : alvo,
      };
    },
