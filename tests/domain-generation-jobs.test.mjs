...
test('A. um banco no esquema 6 migra até o corrente sem perder nada, e sem backfill', () => {
  ...
  try {
    // Um banco como os que já existem em campo: dados reais, esquema anterior.
    {
      ...
      // PASSO 14-D2A: a trilha do projeto sai junto.
      db.exec('DROP TABLE production_music_selections');
      db.exec('DROP TABLE production_music_takes');
      db.exec('DROP TABLE production_music_cues');
      // PASSO 14-D1A: o desenho de som referencia os mesmos três, e sai junto.
      db.exec('DROP TABLE production_scene_sfx_selections');
      db.exec('DROP TABLE production_scene_sfx_takes');
      db.exec('DROP TABLE production_scene_sfx_cues');
