...
/**
 * Liga um Asset já registrado ao job que o produziu.
 *
 * Existe por causa de uma identidade dupla no domínio: um arquivo publicado é
 * identificado por `(projectId, filename)` — a chave do backfill e do índice
 * único — e uma geração é identificada por `(projectId, jobId)`. Como o nome do
 * arquivo publicado É o jobId mais a extensão, as duas chaves apontam para a
 * mesma coisa física; mas um registro criado pelo backfill pode ter chegado
 * primeiro, e nesse caso o Asset existe sem que ninguém tenha anotado de qual
 * job ele veio.
 *
 * Adotar é o certo aí: o arquivo é o mesmo, e criar um segundo Asset para ele
 * seria duplicar um arquivo físico que só existe uma vez.
 *
 * Recusa adotar um Asset que já pertence a OUTRO job. Isso não é conflito de
 * corrida, é dois jobs reivindicando o mesmo arquivo — que, dado como o nome é
 * formado, significa que alguma premissa quebrou. Falhar alto é a resposta.
 */
export function linkAssetToJob(assetId, jobId, db = database()) {
  const alvo = getAsset(assetId, db);
  if (!alvo) throw new DomainError(`Asset desconhecido: "${assetId}".`, { assetId });

  const novo = textoOuNulo(jobId);
  if (!novo) throw new DomainError('jobId é obrigatório para ligar um asset.', { assetId });

  if (alvo.jobId && alvo.jobId !== novo) {
    throw new DomainError(
      `O arquivo já está registrado para outra geração no projeto "${alvo.projectId}".`,
      { assetId, jobIdAtual: alvo.jobId, jobIdPedido: novo },
    );
  }
  if (alvo.jobId === novo) return alvo;

  db.prepare('UPDATE assets SET jobId = ? WHERE id = ?').run(novo, assetId);
  return getAsset(assetId, db);
}
