...
// ── a mídia que ESTA conversa produziu ──────────────────────────────────────
//
// ── Por que isto precisa existir ────────────────────────────────────────────
//
// "Anime essa imagem" é o pedido mais natural do produto, e ele exige que a
// ferramenta receba `sourceAssetId`. Só que o modelo NÃO TEM esse id: quando
// ele pede uma geração, a ferramenta devolve apenas `{ jobId, kind, status }` —
// o Asset ainda não existe. Ele nasce depois, no acompanhamento do servidor,
// quando o turno do modelo já acabou.
//
// O resultado media-se no banco: o agente entendia "essa imagem", não tinha id
// nenhum para passar, e fazia a única coisa que podia — descrever a imagem num
// prompt e gerar um vídeo do zero. O vídeo parecia certo e a linhagem nascia
// nula, porque não havia linhagem: não houve imagem→vídeo.
//
// Esta função é a resposta: o SERVIDOR diz ao modelo quais Assets ele pode
// referenciar. O modelo nunca inventa nem lembra um id — ele escolhe dentro de
// uma lista que o servidor montou a partir do que a conversa realmente produziu.
//
// ── Por que a THREAD, e não o Project ───────────────────────────────────────
//
// Porque "essa imagem" é dêitico: ele aponta para o que está à vista, e o que
// está à vista é esta conversa. Uma conversa nova não deve herdar o referente
// da anterior — pela mesma razão que ela não herda o anexo de um turno.
//
// ── O que sai daqui ─────────────────────────────────────────────────────────
//
// `assetId` (que já é público — a tela o recebe em `tool.completed` e na
// releitura da conversa), `kind`, e o `prompt` que gerou a mídia. O prompt é o
// que torna dois Assets DISTINGUÍVEIS: sem ele, uma lista de identificadores
// não ajuda ninguém a decidir qual é "a do astronauta". Ele é texto que o
// próprio modelo escreveu — devolvê-lo não conta nada novo a ninguém.
//
// Nada de caminho, nada de jobId, nada de workflow.

/** Teto do que é oferecido como referência num turno. */
const MAX_ASSETS_REFERENCIAVEIS = 6;

/**
 * A mídia que esta conversa produziu, mais recente primeiro.
 *
 * `kind` filtra ('image' ou 'video'); omitir traz as duas. O limite existe
 * porque isto entra no contexto de um modelo: uma conversa longa tem dezenas de
 * Assets, e oferecer todos gastaria o turno para listar coisas que ninguém vai
 * referenciar.
 */
export function listThreadAssets(threadId, opcoes = {}, db = database()) {
  const { kind = null, limit = MAX_ASSETS_REFERENCIAVEIS } = opcoes;
  if (typeof threadId !== 'string' || !threadId) return [];

  const teto = Math.max(1, Math.min(Number(limit) || MAX_ASSETS_REFERENCIAVEIS, 50));

  // A ordem é a da CONVERSA (`seq` da mensagem), não a do relógio: duas
  // gerações do mesmo turno cabem no mesmo milissegundo, e nesse caso
  // `createdAt` não ordena nada. É a mesma razão pela qual a conversa inteira é
  // lida por `seq`.
  const linhasBrutas = linhas(db.prepare(`
    SELECT a.id, a.kind, a.prompt, m.seq
      FROM agent_message_assets ma
      JOIN agent_messages m ON m.id = ma.messageId
      JOIN assets a         ON a.id = ma.assetId
     WHERE m.threadId = ?
       AND (? IS NULL OR a.kind = ?)
     ORDER BY m.seq DESC, ma.seq DESC
     LIMIT ?
  `).all(threadId, kind, kind, teto));

  return linhasBrutas.map((registro) => ({
    assetId: registro.id,
    kind: registro.kind,
    // Truncado: o que o modelo precisa é reconhecer a mídia, não reler o
    // prompt inteiro. Um prompt de mil caracteres por Asset encheria o turno.
    prompt: typeof registro.prompt === 'string' && registro.prompt.trim()
      ? registro.prompt.trim().slice(0, 160)
      : null,
  }));
}
