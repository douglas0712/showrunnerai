...
// pedido.
//
// Ele **não consulta a fila** do executor, e por isso **não declara nada como
// perdido**. Um trabalho que não aparece no histórico pode muito bem estar
// enfileirado ou executando agora — chamá-lo de órfão seria transformar
// desconhecimento em desfecho. Esses ficam abertos, e é o passo seguinte que
// vai olhar a fila e decidir.
//
// Ele **não recria acompanhamento**. Esta etapa é sobre o que já acabou.
//
...
import { historyList } from '../comfy/client.js';
...
export async function reconcileGenerationJobs({ db = database(), deps = {} } = {}) {
  ...
  const abertos = listOpenGenerationJobs({}, db);
  ...
  let entradas;
  try {
    entradas = await historico(janelaDeHistorico(abertos.length));
  } catch (erro) {
    ...
    logWarn(STAGES.GENERATION_LEDGER, 'O executor não respondeu; a reconciliação fica para depois.', {
      channel: CHANNELS.COMFY,
      detail: { abertos: abertos.length, causa: erro?.message || String(erro) },
    });
    return {
      recuperados: [], falhados: [], abertos: abertos.map((j) => j.jobId), indisponivel: true,
    };
  }

  const porJobId = indexarPorJobId(entradas, abertos);
  ...
  for (const job of abertos) {
    ...
    const desfecho = await reconciliarUm(job, {
      entradas, porJobId, db, concluir, localizarArquivo,
    });
  ...
  logInfo(STAGES.GENERATION_LEDGER, 'Reconciliação de arranque concluída.', {
    ...
    detail: {
      ...
      // Continuam abertos porque podem estar na fila ou executando agora. Quem
      // olha a fila é o passo seguinte.
      seguemAbertos: seguemAbertos.length,
    },
  });

  return { recuperados, falhados, abertos: seguemAbertos, indisponivel: false };
}
 ...
 */
function indexarPorJobId(entradas, abertos) {
...
async function reconciliarUm(job, { entradas, porJobId, db, concluir, localizarArquivo }) {
  ...
  const providerJobId = job.providerJobId || doIndice?.promptId || null;

  if (!job.providerJobId && doIndice?.promptId) {
    // A janela de queda. A escrita é única: se outro caminho já anotou outro
    // identificador, isto é recusado em vez de sobrescrever.
    try {
      markGenerationJobSubmitted(job.jobId, doIndice.promptId, { db });
      logInfo(STAGES.GENERATION_LEDGER, 'Identificador do executor recuperado pelo nome do arquivo.', {
        channel: CHANNELS.COMFY,
        jobId: job.jobId,
      });
    } catch (erro) {
  ...
  // ── 5. nada. Continua aberto — pode estar na fila ou executando agora ────
  return 'aberto';
}
