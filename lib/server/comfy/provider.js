...
export async function validarMidia(kind, caminho) {
  ...
  if (kind === 'audio') return validarAudio(caminho);
  return { ok: false, motivo: `tipo de mídia sem validação: ${kind}`, probe: null };
}

/**
 * O arquivo é mesmo áudio? (PASSO 14-D1B)
 *
 * Conferido pelo ffprobe, que ABRE o arquivo — é prova mais forte do que
 * extensão ou número mágico: ele diz se existe FLUXO de áudio e qual a duração.
 * Um HTML de erro renomeado, um arquivo truncado ou um trecho mudo não passam.
 *
 * A ausência de vídeo é exigida junto, e não por preciosismo: um contêiner com
 * imagem dentro é outra coisa, e deixá-lo virar Asset de áudio faria a cena
 * ganhar um "efeito sonoro" que na verdade é um clipe.
 *
 * O contêiner NÃO é restringido a um formato só, ao contrário do vídeo: o
 * `SaveAudio` do ComfyUI publica FLAC, o Piper produz WAV, e converter um no
 * outro por gosto custaria qualidade e uma dependência. Quem decide o que a
 * aplicação sabe servir é `mediaKinds.js`, e a extensão já passou por lá antes
 * de chegar aqui.
 */
export async function validarAudio(caminho) {
  let info;
  try {
  ...
  if (!info.hasAudio) return { ok: false, motivo: 'sem fluxo de áudio', probe: info };
  if (info.hasVideo) return { ok: false, motivo: 'tem fluxo de vídeo', probe: info };
...
export async function finalizeJob(jobId) {
  ...
  return runInJobContext({ jobId, promptId: job.promptId, workflow: caminhoDoWorkflowDoJob(job), iniciadoEm: Date.now() }, async () => {
    ...
    try {
      ...
      ({ bytes, salvo, probe: probeDoArquivo } = await baixarValidarPublicar(job));
    } catch (error) {
    ...
    updateJob(jobId, {
      ...
      result: {
        ...
        bytes: bytes.length,
        // A duração MEDIDA pelo ffprobe na validação. Chega ao Asset pelo
        // caminho de conclusão: para áudio ela é a informação que diz quanto o
        // efeito dura de verdade, e não a que alguém pediu.
        durationSeconds: probeDoArquivo?.duration ?? null,
        sourceFilename: job.comfyOutput.filename,
        sourceSubfolder: job.comfyOutput.subfolder,
      },
    });
