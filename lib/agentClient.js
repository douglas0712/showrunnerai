...
const ROTULOS = Object.freeze({
  ...
  // A leitura de um documento se anuncia, e não mostra NADA do que foi lido.
  // O conteúdo pode ser longo e é material privado do usuário; ele é consumido
  // pelo agente, no servidor. O que a conversa mostra é que houve uma leitura —
  // ver a redução final em lib/server/agent/events.js, que já descarta o
  // resultado de qualquer ferramenta que não produza mídia.
  'project.list_documents': { running: 'Procurando nos documentos…', done: 'Documentos consultados' },
  'project.read_document': { running: 'Lendo o documento…', done: 'Documento lido' },
});
...
export async function streamTurn({
  threadId, content, documentIds = [], onEvent, signal = null, fetchImpl = fetch,
}) {
  const resposta = await fetchImpl('/api/agent/stream', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    // Os anexos vão como IDENTIFICADORES, nunca como bytes: o arquivo já subiu
    // e já é um documento do projeto antes de a mensagem ser enviada. E o
    // servidor não confia nesta lista — ele confere que cada documento existe e
    // é do mesmo projeto da conversa antes de ligá-lo ao turno.
    body: JSON.stringify(
      documentIds?.length ? { threadId, content, documentIds } : { threadId, content },
    ),
    signal,
  });
...
/**
 * Envia um arquivo e devolve o documento que ele virou.
 *
 * O upload acontece ANTES do envio da mensagem, e é isso que permite à tela
 * mostrar o nome do arquivo com o tamanho e o número de páginas antes de a
 * pessoa terminar de escrever — o servidor já leu o arquivo e já sabe.
 *
 * O que volta é metadata: id, nome, tipo, tamanho, páginas. Nunca o texto — o
 * conteúdo não tem rota pública, e quem o lê é o agente, no servidor.
 */
export async function uploadDocument({ projectId, file, fetchImpl = fetch, formDataImpl = FormData } = {}) {
  const corpo = new formDataImpl();
  corpo.append('projectId', projectId);
  corpo.append('file', file);

  const resposta = await fetchImpl('/api/documents', { method: 'POST', body: corpo });
  const dados = await resposta.json().catch(() => ({}));

  if (!resposta.ok) {
    // A mensagem vem do servidor porque é ele quem sabe o que houve — arquivo
    // grande demais, PDF sem texto, tipo não aceito — e todas elas já são
    // frases de produto. Inventar uma genérica aqui apagaria a única coisa
    // acionável que o usuário tem.
    const erro = new Error(dados.error || 'Não consegui anexar este arquivo.');
    erro.code = 'document_upload_failed';
    throw erro;
  }

  return dados.document;
}
