...
import {
  database, DomainError, inteiroOuNulo, linha, linhas, newId, SCENE_MEDIA_KINDS,
  textoOuNulo,
} from './db.js';
// O vocabulário de estados de uma geração. Importado, e não copiado, pelo mesmo
// critério de `ASSET_KINDS`: a situação de um take é DERIVADA do trabalho que o
// produziu, e derivar não pode significar traduzir. `generationJobStates.js`
// não importa nada, então não há ciclo.
import { JOB_STATES } from './generationJobStates.js';
import { getProductionScene } from './production.js';

/**
 * Os tipos de take desta pipeline: imagem e vídeo.
 *
 * Isto era `export { ASSET_KINDS as SCENE_MEDIA_KINDS }` até o PASSO 14-C1, e a
 * troca não é cosmética. As duas listas eram iguais por coincidência, não por
 * contrato — e no instante em que `audio` virou tipo físico de Asset, o alias
 * teria feito esta tabela aceitar `kind='audio'` sem que ninguém decidisse
 * isso. Um take de áudio aqui seria mídia visual que nenhuma tela sabe mostrar,
 * e a narração passaria a ter dois lugares para morar.
 *
 * Áudio não é um estágio da pipeline visual: é um PAPEL, e mora em
 * `sceneAudio.js` desde o PASSO 14-B.
 */
export { SCENE_MEDIA_KINDS };
 ...
 * escolhida pelo usuário debaixo dele. O que NÃO acontece aqui é seleção — esta
 * é a ligação MANUAL, de quem já tem o Asset na mão, e escolher é decisão à
 * parte. A ligação que vem de uma geração concluída é outra função, e essa sim
 * faz a primeira escolha: ver `linkCompletedJobToSceneTake`.
 */
...
export function sceneSelections(projectId, ordinal, db = database()) {
  const saida = {};
  for (const kind of SCENE_MEDIA_KINDS) {
 ...
 *
 * ── Por que esta função existe, e por que ela mora aqui ─────────────────────
 *
 * Porque a geração termina LONGE de quem a começou. O turno da conversa acabou
 * há minutos; quem leva o trabalho até o fim é o acompanhamento, a consulta
 * explícita da tela, ou — depois de um reinício — a reconciliação. Se o vínculo
 * fosse feito por quem chamou a ferramenta, ele existiria só no caminho feliz,
 * e um restart no meio deixaria o take vazio para sempre.
 *
 * Ela é chamada de `completeGenerationJob`, que é o ponto por onde TODOS esses
 * caminhos passam para dizer, duravelmente, "este trabalho acabou e produziu
 * isto". Um lugar só, e o mesmo que já era a fonte da verdade.
 *
 * ── Idempotente de propósito ────────────────────────────────────────────────
 *
 * Concluir é uma operação que se repete: o acompanhamento e uma reconciliação
 * podem chegar os dois. Chamar de novo com o mesmo Asset não faz nada. Chamar
 * com outro Asset é recusado — seria mover a mídia debaixo de quem já a viu.
 *
 * ── A primeira seleção ──────────────────────────────────────────────────────
 *
 * Se a cena ainda não escolheu imagem, o primeiro take que CONCLUIR vira a
 * escolha. Um take que falhou nunca chega aqui: esta função só roda quando há
 * Asset, e Asset só existe em geração concluída.
 *
 * Se já há escolha, ela NÃO muda. `ON CONFLICT DO NOTHING` é a regra inteira:
 * regenerar oferece uma alternativa, e trocar a escolha por baixo transformaria
 * "quero ver outra opção" em "perdi a que eu tinha aprovado". Escolher de novo é
 * decisão de quem assiste.
 *
 * Devolve o take, ou `null` quando a geração não é de cena nenhuma — que é o
 * caso comum: `og.generate_image` continua existindo e não passa por aqui.
 */
export function linkCompletedJobToSceneTake(jobId, assetId, db = database()) {
  const doJob = textoOuNulo(jobId);
  if (!doJob) return null;

  const take = linha(db.prepare(
    'SELECT * FROM production_scene_media WHERE generationJobId = ?',
  ).get(doJob));
  if (!take) return null;
  ...
  // O projeto da cena, subindo a cadeia. Não há coluna a consultar: a cena
  // pertence ao roteiro, e o roteiro ao projeto — é o desenho da migração 9.
  ...
  if (asset.kind !== take.kind) {
    throw new DomainError(
      `O Asset "${id}" é de ${asset.kind}, e este take é de ${take.kind}.`,
      { assetId: id, kindDoAsset: asset.kind, kindDoTake: take.kind },
    );
  }

  if (take.assetId !== null && take.assetId !== id) {
    throw new DomainError(
      `O take ${take.takeNumber} de ${take.kind} já tem outra mídia.`,
      { kind: take.kind, takeNumber: take.takeNumber },
    );
  }
  ...
  // A primeira escolha, e só a primeira. Ver o cabeçalho.
  db.prepare(`
    INSERT INTO production_scene_media_selections (sceneId, kind, mediaId, updatedAt)
    VALUES (?, ?, ?, ?)
    ON CONFLICT (sceneId, kind) DO NOTHING
  `).run(take.sceneId, take.kind, take.id, agora);

  return linha(db.prepare('SELECT * FROM production_scene_media WHERE id = ?').get(take.id));
}

/**
 * A situação de um take, DERIVADA — nunca guardada.
 *
 * ── Por que derivar, e não gravar ───────────────────────────────────────────
 *
 * Porque já existe quem saiba: o `generation_jobs` tem o estado do trabalho, e
 * o Asset diz se o resultado existe. Uma coluna `state` aqui seria uma segunda
 * resposta para a mesma pergunta, e as duas divergiriam no primeiro reinício —
 * a reconciliação atualiza o livro-razão, e não teria por que saber que há uma
 * cópia noutra tabela.
 *
 * ── Nenhuma palavra nova ────────────────────────────────────────────────────
 *
 * O vocabulário é o de `generationJobStates.js`, inteiro e sem tradução. Um
 * take com Asset é `done` porque foi assim que o trabalho terminou; um take sem
 * Asset vale o que o trabalho dele vale agora.
 *
 * `done` para um take que TEM Asset é verdade mesmo sem job — é o caso da mídia
 * já registrada em disco, ligada à mão. O que existe, existe.
 *
 * Devolve `null` só no caso que o produto não produz: take sem Asset e sem
 * trabalho. Não há o que dizer sobre ele, e inventar uma palavra seria inventar
 * o estado junto.
 */
export function sceneTakeState(take, db = database()) {
  if (!take) return null;
  if (take.assetId) return JOB_STATES.DONE;
  if (!take.generationJobId) return null;

  const job = linha(db.prepare(
    'SELECT state FROM generation_jobs WHERE jobId = ?',
  ).get(take.generationJobId));
...
function kindValido(valor) {
  ...
  if (!SCENE_MEDIA_KINDS.includes(kind)) {
    throw new DomainError(
      `Tipo de mídia inválido: "${valor}". Uma cena tem takes de ${SCENE_MEDIA_KINDS.join(' ou ')}.`,
      { kind: valor, aceitos: [...SCENE_MEDIA_KINDS] },
    );
  }
  return kind;
}
