...
export function createHermesRuntime({
  ...
  return {
    ...
    async* run({
      ...
      // O turno é anunciado ao bridge com o contexto que só o Showrunner tem: o
      // sinal de cancelamento e a âncora do turno. Ela vem do `context` que o
      // gateway montou a partir da mensagem que ele acabou de gravar — não do
      // serviço de raciocínio, que nem sabe que ela existe.
      ...
      try {
        ...
        // "não deu para falar com ele" de "ele parou no meio da frase".
        let falou = false;

        let fluxo;
        try {
          fluxo = http.runTurn({ sessionId, message: ultima.content, signal });
