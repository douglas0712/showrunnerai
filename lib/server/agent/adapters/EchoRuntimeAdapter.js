...
export function createEchoRuntime({ clock = Date.now } = {}) {
  ...
  return {
     ...
     */
    async* run({ messages = [], tools = [], signal = null } = {}) {
      // O contrato diz que `tools` chega como coleção; hoje ela vem vazia. A
      // conferência é para que oferecer algo diferente de uma coleção falhe
      // agora, e não no Passo 6.
