...
async function criarThread({ project = null, fetchImpl = fetch }) {
  ...
  return { thread: corpo.thread, messages: corpo.messages || [] };
}
...
export async function startNewThread({
  ...
  const { thread, messages } = await criarThread({ project, fetchImpl });
  ...
  return { thread, messages, criada: true };
}
  ...
  if (threadId) {
    ...
    if (resposta.ok) {
      ...
      if (daThread === projetoAtual) {
        return { thread: corpo.thread, messages: corpo.messages || [], criada: false };
      }
    }
