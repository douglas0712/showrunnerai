...
import { publicToolList, toolRegistry } from './tools/index.js';
import {
 ...
 * Esta função:
 * - Constrói ToolContext confiável (threadId, projectId, signal)
 * - Invoca via registry (único ponto de execução)
 * - Trata erros de forma apropriada
 */
function criarInvokeTool(thread, signal) {
...
export async function sendMessage(entrada = {}, deps = {}) {
  ...
  try {
    ...
    events = await consumirEventos(runtime.run({
      ...
      messages,
      context: contexto,
      // PASSO 6: tools são agora públicas (name, description, inputSchema, sem execute)
      tools,
      // PASSO 6: invokeTool é o único ponto de execução seguro de ferramentas
      invokeTool: criarInvokeTool(thread, signal),
      signal,
    }));
  } catch (erro) {
