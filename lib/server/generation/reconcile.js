...
export async function reconcileGenerationJobs({ db = database(), deps = {} } = {}) {
  ...
  //
  // `listOpenGenerationJobs` filtra por ESTADO, e não por origem — ele é do
  // livro-razão, comum a todos. Esta reconciliação, porém, é a do ComfyUI: ela
  // pergunta ao histórico e à fila DELE, e só pode opinar sobre trabalho que
  // passou por lá.
  //
  // ── Por que o filtro NÃO é por `kind` ───────────────────────────────────
  //
  // Porque `kind` deixou de distinguir origem. Até o PASSO 14-C2 todo job de
  // áudio era do Piper, e `kind !== 'audio'` bastava. O 14-D1B trouxe o efeito
  // sonoro do Stable Audio, que é `kind='audio'` E roda no ComfyUI — filtrar
  // por tipo agora excluiria da reconciliação um trabalho que está vivo nesta
  // fila, e ele ficaria aberto para sempre depois de um reinício.
  //
  // Quem distingue é o WORKFLOW. Um job cujo `workflowId` resolve neste
  // registry passou por aqui; um que não resolve é de outro executor — hoje,
  // a narração, cujo `narration-tts` não é grafo nenhum e por isso não resolve.
  // É a mesma pergunta que `descriptorsDe` já fazia, feita antes.
  const abertos = listOpenGenerationJobs({}, db).filter((job) => esteExecutor(job));
 ...
 */
/**
 * Este trabalho é executado pelo ComfyUI?
 *
 * A resposta é o `workflowId` resolver — ou não — no registry. Nada de lista de
 * tipos a manter em dia: quando um quarto executor aparecer, ele também não vai
 * ter descriptor aqui, e a resposta continua certa sem ninguém editar isto.
 */
function esteExecutor(job) {
  if (!job?.workflowId) return false;
  try {
    return Boolean(getWorkflow(job.workflowId));
  } catch {
    return false;
  }
}
