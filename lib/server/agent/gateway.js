...
import {
  appendMessageRecord, attachMessageAssets, attachMessageDocuments, createThreadRecord,
  getThreadRecord, listMessageAssets, listMessageDocuments, listMessageRecords,
  listThreadAssets, listThreadRecords, MAX_MESSAGE_LENGTH,
} from './threads.js';
...
export function getThread(threadId, deps = {}) {
  ...
  const messages = listMessageRecords(threadId, db).map((mensagem) => ({
    ...
    // Os documentos que o usuário anexou àquele turno. Metadata segura, e só
    // ela: nome, tipo, páginas, tamanho. O CONTEÚDO não sai por aqui — quem o
    // lê é o agente, server-side, pela ferramenta. Uma leitura de tela que
    // trouxesse o texto criaria um segundo caminho para o mesmo dado, com a
    // fronteira feita de novo.
    documents: listMessageDocuments(mensagem.id, db),
  }));
...
export async function* streamMessage(entrada = {}, deps = {}) {
  ...
  const { threadId, content, documentIds = [], signal = null } = entrada;
  ...
  // O vínculo com os documentos anexados é gravado ANTES de o runtime pensar.
  //
  // A ordem é a garantia inteira: o que o agente vai raciocinar sobre este
  // turno precisa já ser um fato do banco, não uma intenção em memória. Se o
  // turno falhar no meio, a fala do usuário permanece — e o anexo dela também,
  // porque ele foi realmente anexado.
  //
  // `attachMessageDocuments` confere tudo: o documento existe, é do mesmo
  // projeto da conversa, e a mensagem é desta thread. O navegador diz o que
  // anexou; ele não decide se pode.
  if (Array.isArray(documentIds) && documentIds.length) {
    attachMessageDocuments(userMessage.id, documentIds, db);
  }
  // Relido do BANCO, e não montado a partir do que o navegador mandou: o que
  // vale é o que a associação aceitou. Um id de outro projeto foi descartado
  // lá, e não pode reaparecer aqui.
  const anexosDoTurno = listMessageDocuments(userMessage.id, db);
  ...
  const contexto = {
    ...
    // Os documentos anexados A ESTE TURNO, em metadata segura.
    //
    // Sem isto, "sobre o que é este documento?" não teria referente: o modelo
    // veria só o texto da pergunta. Com isto, ele sabe QUAL arquivo o usuário
    // acabou de entregar, e pode lê-lo pela ferramenta.
    //
    // O que atravessa é nome, tipo, páginas, tamanho e o identificador — nunca
    // caminho, nunca impressão digital, e nunca o texto. O conteúdo é grande e
    // é privado; ele é lido sob demanda, e não empurrado para dentro de todo
    // turno.
    //
    // A lista é a que o BANCO aceitou, não a que o navegador mandou. E o texto
    // PÚBLICO da mensagem não é tocado: a mensagem gravada continua sendo
    // exatamente o que o usuário escreveu. Como esta metadata chega ao modelo é
    // decisão de cada adaptador — é a forma do runtime dele que manda.
    attachments: anexosDoTurno,
    // As imagens que ESTA conversa já produziu, para o modelo poder
    // referenciá-las.
    //
    // Sem isto, "anime essa imagem" não tem como virar image-to-video: a
    // ferramenta de geração devolve `{ jobId, kind, status }`, e o Asset só
    // nasce depois, no acompanhamento, com o turno do modelo já encerrado — ele
    // nunca chega a ver um identificador. O que ele fazia era o único caminho
    // que tinha: descrever a imagem num prompt e gerar um vídeo do zero. O
    // vídeo parecia certo e a linhagem nascia nula, porque não havia linhagem.
    //
    // A lista vem do SERVIDOR, da mídia realmente gravada nesta thread. O
    // modelo escolhe dentro dela; ele não inventa nem lembra um id. E é uma
    // LISTA, não "a última": escolher por ele seria heurística, e a heurística
    // erra em silêncio exatamente quando há duas imagens.
    images: listThreadAssets(threadId, { kind: 'image' }, db),
  };
