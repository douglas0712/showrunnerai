...
// A voz (PASSO 14-C2) chega pelo MESMO ponto de conclusão da imagem e do
// vídeo. Um segundo caminho de conclusão seria uma segunda política de
// seleção, e as duas divergiriam no primeiro reinício.
import { linkCompletedJobToNarrationAudioTake } from './sceneAudio.js';
import { linkCompletedJobToSfxTake } from './sceneSfx.js';
import { linkCompletedJobToMusicTake } from './music.js';
import {
...
export function completeGenerationJob(jobId, { assetId, assistantMessageId = undefined, db = database(), at = Date.now() } = {}) {
  ...
  try {
    ...
    // E o mesmo para a narração. As duas são exclusivas na prática — um job
    // produz imagem, vídeo OU voz —, e cada uma devolve `null` quando o job não
    // é dela. Chamar as duas aqui é o que faz watcher, finalize, reconcile e
    // recuperação convergirem para a mesma regra sem repeti-la.
    linkCompletedJobToNarrationAudioTake(registro.jobId, asset, db);
    // E o efeito sonoro (PASSO 14-D1B), pelo mesmo ponto. As três devolvem
    // `null` quando o job não é delas; chamar as três aqui é o que faz watcher,
    // finalize, reconcile e recuperação convergirem sem repetir regra nenhuma.
    linkCompletedJobToSfxTake(registro.jobId, asset, db);
    // E a trilha (PASSO 14-D2B). As quatro devolvem `null` quando o job não é
    // delas; é assim que watcher, finalize, reconcile e recuperação convergem.
    linkCompletedJobToMusicTake(registro.jobId, asset, db);
