...
export const MEDIA_KINDS = Object.freeze({
  ...
  /**
   * A voz de uma cena (PASSO 14-C2).
   *
   * WAV, e não MP3, porque é o que o sintetizador local produz naturalmente —
   * transcodificar por preferência estética custaria uma dependência e uma
   * perda de qualidade para resolver problema nenhum. O dia em que o tamanho
   * incomodar, a conversão é uma decisão consciente, e não um default herdado.
   *
   * `supportsRange` ligado pelo mesmo motivo do vídeo: um player de áudio pede
   * faixas de bytes para poder buscar dentro da faixa sem baixá-la inteira.
   *
   * 'declared': a extensão publicada é a que o provider produziu. Não há
   * detecção por bytes aqui como em imagem porque a validação de áudio é feita
   * pelo ffprobe, que abre o arquivo de verdade — ele é uma prova mais forte do
   * que um número mágico, e já diz se há fluxo de áudio e qual a duração.
   */
  audio: Object.freeze({
    kind: 'audio',
    dir: 'audio',
    ...
    mimeByExtension: Object.freeze({
      '.wav': 'audio/wav',
      // FLAC é o que o `SaveAudio` do ComfyUI produz por padrão, e é o formato
      // em que o efeito sonoro do PASSO 14-D1B chega: sem perda, tocável em
      // todo navegador atual, e a matéria-prima certa para uma mixagem futura.
      // Transcodificar para WAV só por gosto custaria uma dependência e não
      // resolveria problema nenhum.
      '.flac': 'audio/flac',
      '.mp3': 'audio/mpeg',
      '.opus': 'audio/ogg',
    }),
    discoveryExtensions: Object.freeze(['.wav', '.mp3', '.opus', '.ogg', '.flac', '.m4a']),
    extensionSource: 'declared',
  }),
});
