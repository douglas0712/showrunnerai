...
//   jobId: string,
//   kind: "image" | "video",
//   status: string,            // estado de DOMÍNIO do Showrunner:
//                              // "preparing" | "submitted" | "queued" |
//                              // "running" | "finalizing" | "done" |
//                              // "failed" | "cancelled" | "orphaned".
//                              // Nunca o vocabulário do executor.
...
export const getJobTool = defineTool({
  ...
  description: 'Consulta o estado de uma geração (imagem ou vídeo). '
    + 'Responde com um estado entre: preparing, submitted, queued, running, '
  ...
  async execute(context, args) {
    ...
    try {
      ...
      // Era código morto: `status` nunca foi a constante interna do executor,
      // então a condição jamais foi verdadeira. Foi removido em vez de
      // corrigido, porque o que ele fazia a facade já faz — e fazer duas vezes
      // é a maneira de as duas discordarem um dia.
