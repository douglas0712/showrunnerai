...
export const STAGES = {
  ...
  // Fora do fluxo de um job: a conversa do agente. Estas etapas não têm ordem
  // cronológica única nem retentativa — um turno não é uma geração —, então
  // ficam fora de STAGE_ORDER pelo mesmo motivo que CONNECTION_CHECK fica.
  AGENT_THREAD_CREATED: 'AGENT_THREAD_CREATED',
  AGENT_TURN_STARTED: 'AGENT_TURN_STARTED',
  AGENT_RUNTIME_STARTED: 'AGENT_RUNTIME_STARTED',
  AGENT_TURN_COMPLETED: 'AGENT_TURN_COMPLETED',
  AGENT_TURN_FAILED: 'AGENT_TURN_FAILED',
  // Fora do fluxo de um turno: o acompanhamento de uma geração que o agente
  // começou e que continua depois de o turno ter acabado. É trabalho do
  // servidor, não da conversa, e por isso tem etapas próprias.
  AGENT_JOB_WATCH_STARTED: 'AGENT_JOB_WATCH_STARTED',
  AGENT_JOB_WATCH_PROGRESS: 'AGENT_JOB_WATCH_PROGRESS',
  AGENT_JOB_WATCH_COMPLETED: 'AGENT_JOB_WATCH_COMPLETED',
  AGENT_JOB_WATCH_FAILED: 'AGENT_JOB_WATCH_FAILED',
  // Fora do fluxo de um job: o livro-razão durável das gerações. Falhar em
  // ANOTAR não é falhar em GERAR, e por isso tem etapa própria — misturar as
  // duas faria um problema de banco parecer um problema de GPU.
  GENERATION_LEDGER: 'GENERATION_LEDGER',
  // Fora do fluxo de um job e fora do fluxo de um turno: a entrada de material
  // de referência no projeto. Não é geração — nada é produzido — e não é
  // conversa: o usuário pode anexar um PDF sem dizer uma palavra.
...
export const STAGE_LABELS = {
  ...
  [STAGES.AGENT_THREAD_CREATED]: 'Conversa criada',
  [STAGES.AGENT_TURN_STARTED]: 'Turno iniciado',
  [STAGES.AGENT_RUNTIME_STARTED]: 'Agente pensando',
  [STAGES.AGENT_TURN_COMPLETED]: 'Resposta concluída',
  [STAGES.AGENT_TURN_FAILED]: 'Turno falhou',
  [STAGES.AGENT_JOB_WATCH_STARTED]: 'Acompanhando a produção',
  [STAGES.AGENT_JOB_WATCH_PROGRESS]: 'Produção avançou',
  [STAGES.AGENT_JOB_WATCH_COMPLETED]: 'Produção concluída',
  [STAGES.AGENT_JOB_WATCH_FAILED]: 'Produção não concluída',
  [STAGES.GENERATION_LEDGER]: 'Registro durável da geração',
  [STAGES.DOCUMENT_INGESTED]: 'Documento anexado',
};
...
export function explainFailure(stage, message = '') {
  ...
  const porEtapa = {
    ...
    [STAGES.AGENT_TURN_FAILED]:
      'O Showrunner não conseguiu responder. Sua mensagem foi guardada; nada foi inventado no lugar da resposta.',
  };
