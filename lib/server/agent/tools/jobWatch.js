...
/**
 * O estado público de um job, a partir do status que a facade devolve.
 *
 * Só `salvando` vira "finalizando": é o momento em que o arquivo está sendo
 * publicado, e é a única fase intermediária que o usuário percebe como
 * diferente de "gerando". Todo o resto do caminho — preparar, enfileirar,
 * gerar, decodificar — é a mesma coisa do ponto de vista de quem espera.
 */
export function estadoDeProducao(status) {
  if (status === 'concluido') return PRODUCAO.CONCLUIDO;
  if (status === 'falhou' || status === 'cancelado') return PRODUCAO.FALHOU;
  if (status === 'salvando') return PRODUCAO.FINALIZANDO;
...
export function criarRegistroDeAcompanhamento({
  ...
  async function acompanhar(registro, db) {
    ...
    for (;;) {
      ...
      const estado = estadoDeProducao(resultado.status);
      ...
      if (estado === PRODUCAO.FALHOU) {
        assentar(registro, PRODUCAO.FALHOU, db, resultado.error || 'a geração não concluiu');
        return;
      }
