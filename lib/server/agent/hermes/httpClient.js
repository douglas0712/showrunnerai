...
export function createHermesClient({
  ...
  return {
    ...
    /**
     * Aplica a identidade do Showrunner à sessão.
     *
     * O runtime resolve a persona pelo NOME e a entrega ao modelo como instrução
     * de sistema, antes de gerar — não é mensagem da conversa, e por isso não
     * aparece no histórico nem chega ao navegador.
     *
     * Falhar aqui é motivo para não usar a sessão. Um agente que responde sem
     * saber quem é responde como o produto de outra pessoa, e foi exatamente
     * isso que este caminho existe para impedir.
     */
    async applyPersona({ sessionId, name, signal = null } = {}) {
      const carga = await pedir('/api/personality/set', {
        method: 'POST',
        body: { session_id: String(sessionId), name: String(name) },
        signal,
      });

      const aplicado = carga?.personality ?? null;
      if (aplicado !== String(name)) {
        throw new HermesTransportError('A identidade do agente não pôde ser aplicada.', {
          pedido: String(name), aplicado,
        });
      }
      return { personality: aplicado };
    },
